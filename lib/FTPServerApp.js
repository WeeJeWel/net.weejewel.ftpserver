...
module.exports = class FTPServerApp extends Homey.App {
  ...
  async onInit() {
    // Place all userdata, to prevent public hosting, in a unique folder
    ...
    this.ftpServer.on('login', ({
      ...
      Promise.resolve().then(async () => {
        ...
        // User's root folder
        const root = path.join(this.dir, user.root);
    ...
    this.httpServer.use(express.static(this.dir));
    ...
    try {
      this.homey.dashboards
        .getWidget('html')
        .registerSettingAutocompleteListener('file', async (query, settings) => {
          // Find all files endings with .html recursively,
          // and return them as /path/to/file.html

          const files = await fse.readdir(this.dir, { recursive: true });
          files.sort();
    ...
    if (timelineNotificationWelcomeCreated) {
      await this.homey.notifications.createNotification({
        excerpt: 'Welcome to HTTP/FTP Server! Visit the app\'s settings to create your first FTP user.',
      });
