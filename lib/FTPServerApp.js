...
module.exports = class FTPServerApp extends Homey.App {
  ...
  async onInit() {
    ...
    const secureId = Promise.resolve().then(async () => {
    ...
    this.ftpServer.on('login', ({
      ...
      Promise.resolve().then(async () => {
        ...
        if (username === 'anonymous') {
          const anonymousEnabled = await this.homey.settings.get('anonymousEnabled') || false;
          if (!anonymousEnabled) {
            return reject(new FtpSrv.errors.GeneralError('Invalid username', 401));
          }
        ...
        if (user.password !== password) {
          return reject(new FtpSrv.errors.GeneralError('Invalid password', 401));
        }

        // User's root folder
        const root = path.join(dir, user.dir);
